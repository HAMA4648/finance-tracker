  ...
  return (
    <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm mb-8">
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={() => setShowIssueModal(true)} className="bg-indigo-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-indigo-700 transition-colors shadow-sm">
          + Issue Loan
        </button>
      </div>
      ...
      {showIssueModal && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
            ...
            <form action={issueLoan} className="flex flex-col gap-4">
              <div>
              ...
              <div className="flex gap-3 mt-4">
                <button type="submit" onClick={() => setTimeout(() => setShowIssueModal(false), 100)} className="flex-1 bg-slate-900 text-white font-medium py-2 rounded-lg">Issue Loan</button>
            ...
            <div key={loan.id} className="p-4 rounded-xl border border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-4 relative">
              ...
              {isActive && (
                <div className="flex gap-2 relative">
                  ...
                  {repayModal === loan.id && (
                    <div className="absolute right-0 top-0 bottom-0 bg-white/95 backdrop-blur-sm p-2 rounded-xl z-10 flex items-center border border-slate-200 shadow-lg min-w-[250px]">
                      <form action={addLoanRepayment} className="flex gap-2 w-full">
                        ...
                        <button type="submit" onClick={() => setTimeout(() => setRepayModal(null), 100)} className="bg-slate-900 text-white text-sm font-medium px-3 py-2 rounded-lg">Log</button>
